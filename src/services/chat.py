...
class ChatService:
    ...
    async def create_chat_message(
        self, message: str, user_id: UUID, username: str
    ) -> Optional[ChatMessageGet]:
        try:
            input_data = ChatMessage(message=message, user_id=user_id)
            chat_message = await self.repository.add(record=input_data)
            await self.repository.commit()
            return self.repository.to_schema(chat_message, username)
        except Exception as e:
            logger.error(str(e))
            return None

    async def get_recent_chat_messages(
        self, filters: RecentChatMessagesFiltersGet
    ) -> list[ChatMessageGet]:
